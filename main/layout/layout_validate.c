...
static const int GRAPH_BAR_BUCKET_MIN_ALLOWED[] = {
    5,
    10,
    15,
    30
    ...
    const char *domain)
{
    ...
    return
        strncmp(entity_id, domain, domain_len) == 0 &&
...
static bool is_supported_page_type(const char *type)
{
    ...
    return
        strcmp(type, "dashboard") == 0 ||
        strcmp(type, "energy_dashboard") == 0;
}
...
static widget_size_limits_t widget_size_limits_for_type(
    const char *type)
{
    ...
    if (strcmp(type, "sensor") == 0 || strcmp(type, "binary_sensor") == 0 || strcmp(type, "person") == 0 || strcmp(type, "device_tracker") == 0) {
    ...
    } else if (strcmp(type, "slider") == 0 || strcmp(type, "input_number") == 0 || strcmp(type, "select") == 0 || strcmp(type, "input_text") == 0 || strcmp(type, "input_datetime") == 0 || strcmp(type, "alarm_control_panel") == 0 || strcmp(type, "update") == 0) {
        ...
        /*
         * Timer tile sizing mirrors the runtime widget sizing.
         * Keep the validator and runtime constraints identical
         * so a layout accepted here can actually be rendered.
         */
#if defined(CONFIG_APP_PANEL_VARIANT_S3_480)
    ...
    const char *entity_id)
{
    ...
    if (strcmp(type, "button") == 0) {
        return
            ...
            entity_in_domain(entity_id, "button") ||
    ...
    const char *required_domain =
        required_domain_for_widget_type(type);
...
static bool widget_requires_primary_entity(
    const char *type)
{
    return
        type == NULL ||
...
static bool is_valid_slider_direction(
    const char *direction)
{
    ...
    return
        strcmp(direction, "auto") == 0 ||
        strcmp(direction, "left_to_right") == 0 ||
        strcmp(direction, "right_to_left") == 0 ||
        strcmp(direction, "bottom_to_top") == 0 ||
        strcmp(direction, "top_to_bottom") == 0;
}

static bool is_valid_button_mode(
    const char *mode)
{
...
static bool button_mode_requires_runnable(
    const char *mode)
{
    return
        mode != NULL &&
    ...
    const char *source)
{
    ...
    return
        strcmp(source, "ha_energy") == 0 ||
        strcmp(source, "manual_live") == 0;
}

static void validate_energy_entity_field(
    cJSON *energy,
    const char *key,
    const char *page_id,
    layout_validation_result_t *result)
{
    ...
    if (!cJSON_IsString(item) ||
        ...
        snprintf(
            msg,
            sizeof(msg),
            "page %s energy.%s must be a string",
    ...
    if (!is_valid_entity_id(item->valuestring) ||
        !entity_in_domain(
            item->valuestring,
            "sensor")) {
        ...
        snprintf(
            msg,
            sizeof(msg),
            "page %s energy.%s must be sensor.*",
            page_id != NULL ? page_id : "?",
            key);
...
static void validate_energy_page(
    cJSON *page,
    const char *page_id,
    layout_validation_result_t *result)
{
    ...
    if (source != NULL) {
        ...
        if (!cJSON_IsString(source) ||
            ...
            snprintf(
                msg,
                sizeof(msg),
                "page %s energy.source must be a string",
                page_id != NULL ? page_id : "?");
            ...
            !is_valid_energy_source(
                source->valuestring)) {

            snprintf(
                msg,
                sizeof(msg),
                "page %s energy.source must be "
                "ha_energy or manual_live",
                page_id != NULL ? page_id : "?");
    ...
    layout_validation_result_t *result)
{
    ...
    memset(
        result->messages,
        0,
        sizeof(result->messages));
}

void layout_validation_add(
    layout_validation_result_t *result,
    const char *msg)
{
    ...
    snprintf(
        result->messages[result->count],
        sizeof(result->messages[result->count]),
        "%s",
        msg);
...
static bool validate_widget(
    cJSON *widget,
    const char *known_widget_ids,
    size_t known_ids_len,
    size_t page_index,
    size_t widget_index,
    layout_validation_result_t *result)
{
    ...
    if (!cJSON_IsString(id) ||
        ...
        APP_MAX_WIDGET_ID_LEN) {

        snprintf(
            msg,
            sizeof(msg),
            "widget id too long: %s",
            id->valuestring);
            ...
            if (!widget_entity_domain_valid(
                ...
                if (strcmp(
                    ...
                    snprintf(
                        ...
                        sizeof(msg),
                        "widget %s: entity_id must be "
                        "switch|media_player|script|scene.*",
                        cJSON_IsString(id)
                            ? id->valuestring
                            : "?");
                    ...
                    const char *required_domain =
                        required_domain_for_widget_type(
                            type->valuestring);

                    snprintf(
                        msg,
                        sizeof(msg),
                        "widget %s: entity_id must be %s.*",
                        cJSON_IsString(id)
                            ? id->valuestring
                            : "?",
        ...
        if (cJSON_IsString(secondary_entity_id) &&
            ...
            if (!is_valid_entity_id(
                ...
                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: invalid secondary_entity_id",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?");
        ...
        if (cJSON_IsString(style_variant) &&
            ...
            if (strcmp(
                ...
                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: style_variant must be "
                    "default|arc_semi",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?");
    ...
    if (cJSON_IsString(type) &&
        ...
        if (slider_direction != NULL) {
            ...
            if (!cJSON_IsString(slider_direction) ||
                ...
                !is_valid_slider_direction(
                    slider_direction->valuestring)) {

                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: slider_direction must be "
        ...
        if (slider_accent_color != NULL) {
            ...
            if (!cJSON_IsString(slider_accent_color) ||
                ...
                !is_valid_hex_rgb_color(
                    slider_accent_color->valuestring)) {

                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: slider_accent_color "
                    "must be hex RGB",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?");
    ...
    if (cJSON_IsString(type) &&
        ...
        if (button_mode != NULL) {
            ...
            if (!cJSON_IsString(button_mode) ||
                ...
                !is_valid_button_mode(
                    button_mode->valuestring)) {

                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: button_mode must be "
                    "auto|run|play_pause|stop|next|previous",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?");
                ...
                button_mode_requires_media_player(
                    button_mode->valuestring) &&
                cJSON_IsString(entity_id) &&
                entity_id->valuestring != NULL &&
                !entity_in_domain(
                ...
                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: button_mode %s requires "
                ...
                button_mode_requires_runnable(
                    button_mode->valuestring) &&
                !entity_is_runnable_type) {

                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: button_mode run requires "
                    "script.* or scene.* entity_id",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?");
                ...
                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: script.*/scene.* entity_id "
                    "requires button_mode run",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?");
    ...
    if (cJSON_IsString(type) &&
        ...
        if (graph_point_count != NULL) {
            ...
            if (!cJSON_IsNumber(graph_point_count) ||
                graph_point_count->valuedouble <
                    (double)GRAPH_POINT_COUNT_MIN ||
                graph_point_count->valuedouble >
                    (double)GRAPH_POINT_COUNT_MAX ||
                (double)graph_point_count->valueint !=
                    graph_point_count->valuedouble) {

                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: graph_point_count must be "
                    "integer %d..%d",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?",
        ...
        if (graph_time_window_min != NULL) {
            ...
            if (!cJSON_IsNumber(graph_time_window_min) ||
                graph_time_window_min->valuedouble <
                    (double)GRAPH_TIME_WINDOW_MIN_MIN ||
                graph_time_window_min->valuedouble >
                    (double)GRAPH_TIME_WINDOW_MIN_MAX ||
                (double)graph_time_window_min->valueint !=
                    graph_time_window_min->valuedouble) {

                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: graph_time_window_min must be "
                    "integer %d..%d",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?",
        ...
        if (graph_display_mode != NULL) {
            ...
            if (cJSON_IsString(graph_display_mode) &&
                ...
                for (
                    size_t i = 0;
                    i <
                        sizeof(GRAPH_DISPLAY_MODES) /
                        sizeof(GRAPH_DISPLAY_MODES[0]);
                    ++i) {

                    if (strcmp(
                            graph_display_mode->valuestring,
                            GRAPH_DISPLAY_MODES[i]) == 0) {
            ...
            if (!valid_mode) {
                ...
                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: graph_display_mode must be "
                    "line|line_smooth|line_smooth_points|bars",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?");
        ...
        if (graph_bar_bucket_min != NULL) {
            ...
            if (cJSON_IsNumber(graph_bar_bucket_min) &&
                (double)graph_bar_bucket_min->valueint ==
                    graph_bar_bucket_min->valuedouble) {

                for (
                    size_t i = 0;
                    i <
                        sizeof(GRAPH_BAR_BUCKET_MIN_ALLOWED) /
                        sizeof(GRAPH_BAR_BUCKET_MIN_ALLOWED[0]);
                    ++i) {

                    if (
                        graph_bar_bucket_min->valueint ==
                        GRAPH_BAR_BUCKET_MIN_ALLOWED[i]) {
            ...
            if (!valid_bucket) {
                ...
                snprintf(
                    msg,
                    sizeof(msg),
                    "widget %s: graph_bar_bucket_min must be "
                    "one of 5|10|15|30",
                    cJSON_IsString(id)
                        ? id->valuestring
                        : "?");
    ...
    if (!cJSON_IsObject(rect)) {
        ...
        if (!cJSON_IsNumber(x) ||
            ...
            snprintf(
                msg,
                sizeof(msg),
                "widget %s: rect values must be numbers",
                cJSON_IsString(id)
                    ? id->valuestring
                    : "?");
    ...
    layout_validation_result_t *result)
{
    ...
    char *known_widget_ids =
        calloc(
            APP_MAX_WIDGETS_TOTAL,
            APP_MAX_WIDGET_ID_LEN);

    if (known_page_ids == NULL ||
        known_widget_ids == NULL) {
    ...
    for (int i = 0;
        ...
        if (page_type != NULL) {
            ...
            if (!cJSON_IsString(page_type) ||
                page_type->valuestring == NULL ||
                !is_supported_page_type(
                    page_type->valuestring)) {

                snprintf(
                    msg,
                    sizeof(msg),
                    "page %s: unsupported type",
                    cJSON_IsString(page_id)
                        ? page_id->valuestring
                        : "?");
        ...
        if (is_energy_dashboard_page) {
            ...
            if (widgets != NULL &&
                !cJSON_IsArray(widgets)) {

                snprintf(
                    msg,
                    sizeof(msg),
                    "page %s: widgets must be array",
                    cJSON_IsString(page_id)
                        ? page_id->valuestring
                        : "?");
                ...
                cJSON_IsArray(widgets) &&
                cJSON_GetArraySize(widgets) > 0) {

                snprintf(
                    msg,
                    sizeof(msg),
                    "page %s: energy_dashboard "
                    "pages cannot contain widgets",
                    cJSON_IsString(page_id)
                        ? page_id->valuestring
                        : "?");
        ...
        if (widget_count >
            APP_MAX_WIDGETS_PER_PAGE) {

            snprintf(
                msg,
                sizeof(msg),
                "page %s: too many widgets",
                cJSON_IsString(page_id)
                    ? page_id->valuestring
                    : "?");
