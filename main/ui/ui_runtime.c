...
static esp_err_t ui_runtime_alloc_buffers(void)
{
    ...
    if (s_energy_pages == NULL) {
        ...
        if (s_energy_pages == NULL) {
            ESP_LOGE(TAG_UI, "Failed to allocate energy page runtime buffer (%u pages)",
                     (unsigned)APP_MAX_PAGES);
            return ESP_ERR_NO_MEM;
        }
    }
...
static void ui_runtime_refresh_topbar(void)
{
    ...
    bool datetime_changed =
        !s_topbar_cache.valid ||
        info.tm_min != s_topbar_cache.minute ||
        info.tm_hour != s_topbar_cache.hour ||
        info.tm_mday != s_topbar_cache.day ||
        info.tm_mon != s_topbar_cache.month ||
        info.tm_year != s_topbar_cache.year;

    bool status_changed =
        !s_topbar_cache.valid ||
        wifi_connected != s_topbar_cache.wifi_connected ||
    ...
    if (status_changed) {
        ui_pages_set_topbar_status(
            wifi_connected,
            wifi_setup_ap_active,
            ha_connected,
            ha_initial_sync_done);
    }
...
static void ui_runtime_show_weather_icon_overlay(void)
{
    ...
    lv_obj_add_flag(
        s_weather_icon_overlay,
        LV_OBJ_FLAG_IGNORE_LAYOUT);

    lv_obj_set_style_text_color(
        s_weather_icon_overlay,
        lv_color_hex(0x2FE3E3),
        LV_PART_MAIN);
    ...
    ESP_LOGI(
        TAG_UI,
        "Weather icon overlay test enabled (font=%s)",
        (mdi_font_weather() != NULL) ? "72/56" : "none");
#endif
}

static void ui_runtime_apply_entity_state_ex(
    const char *entity_id,
    bool mark_unavailable_if_missing)
{
    ...
    for (size_t i = 0; i < s_widget_count; i++) {
        bool is_primary =
            (strncmp(
                entity_id,
                s_widgets[i].entity_id,
                APP_MAX_ENTITY_ID_LEN) == 0);

        bool is_secondary =
            (s_widgets[i].secondary_entity_id[0] != '\0') &&
            (strncmp(
                entity_id,
        ...
        if (found) {
            ...
            ui_widget_factory_mark_unavailable(
                &s_widgets[i]);
        }
    }
    ...
    if (found) {
        for (size_t i = 0; i < s_energy_page_count; i++) {
            (void)ui_energy_page_apply_state(
                &s_energy_pages[i],
                &s_state_scratch);
        }
    }
...
{
    ui_runtime_apply_entity_state_ex(
        entity_id,
        true);
}

static void ui_runtime_apply_widget_current_state(
    ui_widget_instance_t *widget,
    bool mark_unavailable_if_missing)
{
    if (widget == NULL) {
        return;
    ...
    bool refresh_visible_widgets)
{
    ...
    for (size_t i = 0; i < s_widget_count; i++) {
        ...
        if (visible && refresh_visible_widgets) {
            ui_runtime_apply_widget_current_state(
                &s_widgets[i],
                false);
        }
    }
...
static void ui_runtime_apply_all_states(void)
{
    ...
    for (size_t i = 0; i < s_energy_page_count; i++) {
        ui_energy_page_apply_all_states(
            &s_energy_pages[i]);
    }
}
...
static bool ui_runtime_widget_from_json(
    cJSON *widget_json,
    ui_widget_def_t *out)
{
    if (widget_json == NULL || out == NULL) {
        return false;
    }
    ...
    cJSON *slider_accent_color =
        cJSON_GetObjectItemCaseSensitive(
            widget_json,
            "slider_accent_color");

    cJSON *button_accent_color =
        cJSON_GetObjectItemCaseSensitive(
            widget_json,
            "button_accent_color");
    ...
    cJSON *graph_point_count =
        cJSON_GetObjectItemCaseSensitive(
            widget_json,
            "graph_point_count");

    cJSON *graph_time_window_min =
        cJSON_GetObjectItemCaseSensitive(
            widget_json,
            "graph_time_window_min");

    cJSON *graph_display_mode =
        cJSON_GetObjectItemCaseSensitive(
            widget_json,
            "graph_display_mode");

    cJSON *graph_bar_bucket_min =
        cJSON_GetObjectItemCaseSensitive(
            widget_json,
            "graph_bar_bucket_min");
    ...
    if (cJSON_IsString(secondary_entity_id) &&
        secondary_entity_id->valuestring != NULL) {

        snprintf(
            out->secondary_entity_id,
    ...
    if (cJSON_IsString(slider_direction) &&
        slider_direction->valuestring != NULL) {

        snprintf(
            out->slider_direction,
            sizeof(out->slider_direction),
            "%s",
            slider_direction->valuestring);
    }

    if (cJSON_IsString(slider_accent_color) &&
        slider_accent_color->valuestring != NULL) {

        snprintf(
            out->slider_accent_color,
            sizeof(out->slider_accent_color),
            "%s",
            slider_accent_color->valuestring);
    }

    if (cJSON_IsString(button_accent_color) &&
        button_accent_color->valuestring != NULL) {

        snprintf(
            out->button_accent_color,
    ...
    if (cJSON_IsString(graph_line_color) &&
        graph_line_color->valuestring != NULL) {

        snprintf(
            out->graph_line_color,
            sizeof(out->graph_line_color),
            "%s",
            graph_line_color->valuestring);
    }

    if (cJSON_IsNumber(graph_point_count)) {
        out->graph_point_count =
            graph_point_count->valueint;
    }

    if (cJSON_IsNumber(graph_time_window_min)) {
        out->graph_time_window_min =
            graph_time_window_min->valueint;
    }

    if (cJSON_IsString(graph_display_mode) &&
        graph_display_mode->valuestring != NULL) {

        snprintf(
            out->graph_display_mode,
            sizeof(out->graph_display_mode),
            "%s",
            graph_display_mode->valuestring);
    }

    if (cJSON_IsNumber(graph_bar_bucket_min)) {
        out->graph_bar_bucket_min =
            graph_bar_bucket_min->valueint;
    }

    if (cJSON_IsString(style_variant) &&
        style_variant->valuestring != NULL) {

        snprintf(
            out->style_variant,
            sizeof(out->style_variant),
            "%s",
            style_variant->valuestring);
    }

    if (cJSON_IsString(arc_opening) &&
        arc_opening->valuestring != NULL) {

        snprintf(
            out->arc_opening,
            sizeof(out->arc_opening),
            "%s",
            arc_opening->valuestring);
    }

    /*
     * Icon/display defaults are deliberately enabled when the JSON
     * property is missing. This preserves compatibility with existing
     * layouts created before these properties existed.
     */
    out->show_icon = true;
    out->show_state = true;
    out->show_title = true;
     ...
     * compatibility and so a newly-created timer exposes all controls
     * unless explicitly disabled by the Web UI.
     */
    out->timer_show_start = true;
    out->timer_show_pause = true;
    out->timer_show_cancel = true;
    out->timer_show_finish = true;

    if (cJSON_IsBool(timer_show_start)) {
        out->timer_show_start =
            cJSON_IsTrue(timer_show_start);
    }

    if (cJSON_IsBool(timer_show_pause)) {
        out->timer_show_pause =
            cJSON_IsTrue(timer_show_pause);
    }

    if (cJSON_IsBool(timer_show_cancel)) {
        out->timer_show_cancel =
            cJSON_IsTrue(timer_show_cancel);
    }

    if (cJSON_IsBool(timer_show_finish)) {
        out->timer_show_finish =
            cJSON_IsTrue(timer_show_finish);
    }
...
static void ui_runtime_copy_json_string(
    cJSON *obj,
    const char *key,
    char *dst,
    size_t dst_size)
...
static void ui_runtime_energy_config_from_json(
    cJSON *page_json,
    const char *page_id,
    ...
    ui_energy_page_config_t *out)
{
    ...
    snprintf(
        ...
        sizeof(out->title),
        "%s",
        (page_title != NULL && page_title[0] != '\0')
            ? page_title
            : "Energy");
    ...
    bool source_was_configured =
        ...
        source_item->valuestring != NULL &&
        source_item->valuestring[0] != '\0';
    ...
    if (strcmp(out->source, "manual_live") != 0 &&
        strcmp(out->source, "ha_energy") != 0) {
    ...
    ui_runtime_copy_json_string(
        ...
        out->home_power_entity_id,
        sizeof(out->home_power_entity_id));

    ui_runtime_copy_json_string(
        energy,
        "solar_power_entity_id",
        out->solar_power_entity_id,
        sizeof(out->solar_power_entity_id));

    ui_runtime_copy_json_string(
        energy,
        "grid_power_entity_id",
        out->grid_power_entity_id,
        sizeof(out->grid_power_entity_id));

    ui_runtime_copy_json_string(
        energy,
        "grid_import_power_entity_id",
        out->grid_import_power_entity_id,
        sizeof(out->grid_import_power_entity_id));

    ui_runtime_copy_json_string(
        energy,
        "grid_export_power_entity_id",
        out->grid_export_power_entity_id,
        sizeof(out->grid_export_power_entity_id));

    ui_runtime_copy_json_string(
        energy,
        "battery_power_entity_id",
        out->battery_power_entity_id,
        sizeof(out->battery_power_entity_id));
    ...
    ui_runtime_copy_json_string(
        ...
        out->battery_soc_entity_id,
        sizeof(out->battery_soc_entity_id));

    if (!source_was_configured &&
        (out->home_power_entity_id[0] != '\0' ||
         out->solar_power_entity_id[0] != '\0' ||
         out->grid_power_entity_id[0] != '\0' ||
         out->grid_import_power_entity_id[0] != '\0' ||
         out->grid_export_power_entity_id[0] != '\0' ||
         out->battery_power_entity_id[0] != '\0' ||
         out->battery_charge_power_entity_id[0] != '\0' ||
         out->battery_discharge_power_entity_id[0] != '\0' ||
...
esp_err_t ui_runtime_load_layout(const char *layout_json)
{
    ...
    memset(
        s_widgets,
        0,
        APP_MAX_WIDGETS_TOTAL * sizeof(*s_widgets));
    ...
    memset(
        s_energy_pages,
        0,
        APP_MAX_PAGES * sizeof(*s_energy_pages));

    s_energy_page_count = 0;

    int page_count =
        cJSON_GetArraySize(pages);

    for (int p = 0; p < page_count; p++) {
        cJSON *page =
            cJSON_GetArrayItem(pages, p);
        ...
        bool is_energy_page =
            cJSON_IsString(page_type) &&
            page_type->valuestring != NULL &&
            strcmp(
                page_type->valuestring,
                "energy_dashboard") == 0;
        ...
        if (is_energy_page) {
            ...
            ui_runtime_energy_config_from_json(
                page,
                page_id->valuestring,
                cJSON_IsString(page_title)
                    ? page_title->valuestring
            ...
            esp_err_t err =
                ui_energy_page_create(
                    &energy_config,
                    page_container,
                    &s_energy_pages[s_energy_page_count]);
        ...
        for (int pass = 0; pass < 2; pass++) {
            ...
            for (int w = 0;
                ...
                if (!ui_runtime_widget_from_json(
                        cJSON_GetArrayItem(widgets, w),
                        &def)) {
                    continue;
                }

                bool is_background =
                    ui_runtime_is_background_widget_type(
                        def.type);
                ...
                esp_err_t err =
                    ui_widget_factory_create(
                        &def,
                        page_container,
                        &s_widgets[s_widget_count]);

                if (err == ESP_OK) {
                    snprintf(
                        s_widgets[s_widget_count].page_id,
                        sizeof(s_widgets[s_widget_count].page_id),
                        "%s",
                        page_id->valuestring);
...
esp_err_t ui_runtime_reload_layout(void)
{
     ...
     * newly created widgets. Safe: must be called from the UI task under
     * the display lock, which is the case when triggered via
     * EV_LAYOUT_UPDATED. */
    ...
    esp_err_t err =
        layout_store_load(&json);

    if (err != ESP_OK || json == NULL) {
        json = strdup(
            layout_store_default_json());
...
static void ui_runtime_handle_event(
    const app_event_t *event)
{
    ...
    bool needs_lock =
        (event->type != EV_LAYOUT_UPDATED);

    if (needs_lock && !display_lock(0)) {
        if (event->type == EV_HA_STATE_CHANGED ||
            event->type == EV_HA_CONNECTED ||
        ...
        if ((now_ms - s_deferred_event_log_ms) >= 5000) {
            ESP_LOGW(
                TAG_UI,
                "Deferred UI event processing due to display lock contention (deferred=%u)",
    ...
    case EV_HA_CONNECTED:
        ...
        /* During initial/partial HA sync we may temporarily miss some
         * entities. Preserve currently rendered widgets instead of
         * forcing unavailable. */
...
static void ui_runtime_task(void *arg)
{
    ...
    while (true) {
        ...
        if ((s_pending_state_reconcile ||
            ...
            if (s_pending_state_reconcile) {
                /* Reconcile all states after lock contention so UI never
                 * stays stale. */
                ui_runtime_apply_all_states_preserve_missing();
        ...
        if (s_model_reconcile_pending &&
            (now_ms - s_last_model_reconcile_ms) >=
                UI_MODEL_RECONCILE_INTERVAL_MS &&
            display_lock(20)) {

            /* Protect against missed per-entity events under burst:
             * periodically reconcile from model snapshot. */
            ui_runtime_apply_all_states_preserve_missing();
...
esp_err_t ui_runtime_init(void)
{
    ...
    ui_pages_set_show_callback(
        ui_runtime_on_page_shown);
    ...
    s_last_topbar_refresh_ms =
        esp_timer_get_time() / 1000;

    s_last_model_reconcile_ms =
        s_last_topbar_refresh_ms;

    s_last_model_revision =
        ha_model_state_revision();
